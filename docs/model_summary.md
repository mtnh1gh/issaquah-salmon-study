...
`expm1` and floored at zero). Three interpretable feature sets are tried, plus two
naive comparators:
...
`docs/decision_log.md` D-012, D-013).
...
- **Chinook: no candidate qualifies.** All three environmental models are worse than simply predicting last year's return (best baseline MAE 1,180 vs. every model ≥ 2,038; R² down to -6.36 for the weakest). Chinook results are retained as descriptive association/trend findings only.
- **Coho: 3 of 3 environmental candidates qualify** (`all_environment_ridge`, `freshwater_marine_ols`, `migration_marine_ols`), each beating both Coho baselines on MAE and RMSE — though R² stays negative (-0.17 to -0.52), meaning they're better than a bad baseline, not accurate in an absolute sense.
...
For the three retained Coho models (`src/run_phase5_uncertainty.py`):
...
- Measured empirical coverage was **88.2%** against a nominal 95% target. With only 17 test years, the report explicitly declines to certify calibration and treats the intervals as descriptive, not statistically guaranteed.
